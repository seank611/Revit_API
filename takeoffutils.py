# takeoffutils.py V1.0 09/25/2021
...
def mat_assign(dataframe):
    ...
    conc_keywords = ['Concrete', 'Slab', 'Conc', 'Mat', 'Footing', 'Ftng', 'SOG']
    masonry_keywords = ['Brick', 'Masonry', 'Rubble', 'CMU']
    cfmf_keywords = ['Mtl Stud', 'Metal Stud']
    ...
    for material in material_list:
        ...
        for i in range(dataframe.shape[0]):
            ...
            # if no material defined, run list check against keywords to determine materail type
            if st_mat == 'na':
            ...
            if st_mat == 'Other':
                is_mat = list_check(material_dict.get(material), type, smode=True)
                if is_mat is False:
                    is_mat = list_check(material_dict.get(material), fam, smode=True)
                    if is_mat is True:
                        dataframe.iloc[i, 2] = material
            if material == 'CFMF':
